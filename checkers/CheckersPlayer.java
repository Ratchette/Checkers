					...
					if ((i + j) % 2 == 0) {
						try {
							...
							piece.setPieceImage(scale(
									ImageIO.read(getClass().getResource("/resources/peice8x8.png")), 600/theBoard.getTheBoard().getGridSize(), 600/theBoard.getTheBoard().getGridSize()));
							pieces[count] = piece;
							count ++;
						} catch (Exception e) {
							e.printStackTrace();
				...
				for (int j = 0; j < theBoard.getTheBoard().getGridSize(); j++) {
					if ((i + j) % 2 == 0) {
						try {
							...
							piece.setColour('w'); 
							piece.setPieceImage(scale(
									ImageIO.read(getClass().getResource("/resources/peice8x8w.png")), 600/theBoard.getTheBoard().getGridSize(), 600/theBoard.getTheBoard().getGridSize()));
							pieces[count] = piece;
							count ++;
						} catch (Exception e) {
							e.printStackTrace();
