...
public class Board implements Remote, Serializable {
	...
	public ArrayList<Move> getPossibleMoves(Piece piece) throws Exception {
		...
		if(piece.isCrown()){
			//Basic jump West
			if( checkKingWestDiagonal(piece, 1) && !checkKingWestDiagonal(piece, 2)){
				int x = getKingWestDiagonal(piece, 1).getX();
				int y = getKingWestDiagonal(piece, 1).getY();
				Piece temp = getPieceAtPosition(x, y);
				if(temp.getColour() != piece.getColour()){
					moves.add(new SingleMove(piece, temp, getKingWestDiagonal(piece, 2) ) );
				}
			}
			//Basic West Move logic no jumping
			else if( !checkKingWestDiagonal(piece, 1) ){
				moves.add(new SingleMove(piece, null, getKingWestDiagonal(piece, 1)) );
			}


			//Basic jump East
			if( checkKingEastDiagonal(piece, 1) && !checkKingEastDiagonal(piece, 2)){
				int x = getKingEastDiagonal(piece, 1).getX();
				int y = getKingEastDiagonal(piece, 1).getY();
				Piece temp = getPieceAtPosition(x, y);
				if( piece.getColour() != temp.getColour() ){
					moves.add(new SingleMove(piece, temp, getKingEastDiagonal(piece, 2) ) );
				}
			}
			//Basic East Move logic no jumping
			else if( !checkKingEastDiagonal(piece, 1) ){
				moves.add(new SingleMove(piece, null, getKingEastDiagonal(piece, 1)) );
			}
		}
	...
	//Returns true if position is occupied
	public boolean checkKingWestDiagonal(Piece p, int space){
		Position pos = p.getPiecePosition();
		int x = pos.getX() - space;
		int y = -1;
	...
	private Position getKingWestDiagonal(Piece p, int space){
		Position pos = p.getPiecePosition();
		int x = pos.getX() - space;
		int y = -1;
	...
	public boolean checkKingEastDiagonal(Piece p, int space){
		Position pos = p.getPiecePosition();
		int x = pos.getX() + space;
		int y = -1;
	...
	private Position getKingEastDiagonal(Piece p, int space){
		Position pos = p.getPiecePosition();
		int x = pos.getX() + space;
		int y = -1;
