...
public class Gui implements ActionListener {
	public static final int gridLength = 600;
	...
	public Gui(Board theBoard) {
		...
		board.setSize(gridLength, gridLength);
		menuButtons.setSize(gridLength, 50);
	...
	public void drawBoard(Board theBoard) {
		...
		try {
			for (int i = 0; i < theBoard.getPiecePlacement().length; i++) {
				try {
					int pos = (theBoard.getPiecePlacement()[i]
							.getPiecePosition().getY() * gridSize)
							+ (theBoard.getPiecePlacement()[i]
									.getPiecePosition().getX());
					square[pos].setIcon(new ImageIcon(theBoard
							.getPiecePlacement()[i].getPieceImage()));
				} catch (Exception ex) {
				}
			}
	...
	public void clearGUI() {
		for (int i = 0; i <= ((gridSize * gridSize) - 1); i++) {
			try {
				square[i].setIcon(new ImageIcon(getClass().getResource(
						"/resources/blank.png")));
			} catch (Exception ex) {
			}
		}
	...
	public void actionPerformed(ActionEvent a) {
		...
		/*
		 * for (int i=0; i<boardInfo.getPiecePlacement().length; i++){ int pos =
		 * (boardInfo.getPiecePlacement()[i].getPiecePosition().getY()*gridSize)
		 * + (boardInfo.getPiecePlacement()[i].getPiecePosition().getX());
		 * if(pos == Integer.parseInt(pressedButton.getName())){
		 * 
		 * if(boardInfo.getPiecePlacement()[i].getColour() == Piece.WHITE){
		 * pressedButton.setBorder(new LineBorder(Color.GREEN, 2)); } } }
		 */
		// pressedButton.setBorder(new LineBorder(Color.GREEN, 2));
